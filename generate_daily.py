...
FIGURE_ALIAS = {
    "ph_metabolism": "oxidative",
    "pd_modelcrit": "aerobic_programming",
    "rb_proximal": "speed_agility",
    "ph_altitude": "respiratory",
    "ph_adaptation": "aerobic_programming",
    "ph_hiit": "anaerobic_adaptations",
    "ph_testing": "ex_squat",
    "ph_thermoreg": "cardiovascular",
    "ph_hormones": "anabolic_hormones",
    "ph_o2transport": "respiratory",
    "ph_domains": "aerobic_adaptations",
    "hp_pillars": "needs_analysis",
    "ph_motorunits": "motor_units",
    "ph_mechanics": "force_velocity_length_tension",
...
# NO local state the fallback is a pure guess at RORDER[0] — a lesson he has
# very likely finished — so it is worth waiting out a first-of-day CORS
# preflight + GET on a weak mobile link. The account fuse now CHAINS into the
# gist path (see C1), so the budget is:
...
def build_index_html(base_html, available, this_iso, dtopic=None, rorder=None,
    ...
    redirect = (
        ...
        # perfectly good gist was resumed from local state only and could be
        # thrown backwards onto RORDER[0] — a lesson already finished. Now
        # every account failure falls THROUGH to here. finish()'s single-settle
        # guard makes the chain safe: whichever path answers first wins and the
        ...
        # between the athlete and Day 1 of the relevance order: his progress
        # lives in the worker, not in this browser. Every failure mode — no
        # fetch(), no network, non-200, unparseable body, a record with no
        # .state — now falls through to gistPath() rather than settling, and
        # gistPath()'s own failure path is the pre-existing local-only
        # behaviour. Nothing here can leave the page sitting on the overlay.
...
def build_day(build_date, curriculum, questions, is_entry):
    ...
    try:
        ...
        _today_iso = build_date.isoformat()
        _comp = {_d: _ids for _d, _ids in _comp.items()
                 if _d == _today_iso or (OUT / f"hyrox_{_d}.html").exists()}
    ...
    try:
        # Only list files that actually exist. A phantom entry can never be
        # fetched, so every "Download all" would end with fail >= 1 — and the
        # build stamp is only written on a clean sweep, so the row would read
        # "new lessons available" forever and re-download the whole archive on
        # every tap. (This bit the CSCS twin, which listed modules.html.)
        _pk = []
